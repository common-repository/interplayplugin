	...
	private function parseValue ( $rawValue, $returnType = null )
	{
		...
		if ( substr( $rawValue, 0, 2 ) == '${' && substr( $rawValue, -1 ) == '}' ) {
			...
			if ( array_key_exists( $propertyPlaceholder, $beanFactoryDefaultProperties ) ) {
				if ( $this->isSimpleValue( $beanFactoryDefaultProperties[$propertyPlaceholder] ) ) {
					return $beanFactoryDefaultProperties[$propertyPlaceholder];
				}
				else {
					throw new Exception( 'The supplied value for property placeholder ' . $propertyPlaceholder . ' is not a simple type. This error occured while while resolving properties with the default bean factory properties!' );
				}
			}
		}
