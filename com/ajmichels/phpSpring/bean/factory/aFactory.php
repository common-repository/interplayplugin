			...
			if ( !$beanDef->isConstructed() ) {
				...
				foreach ( $propDefs as $propDef ) {
					...
					if ( $propType == 'value' ) {
						...
						call_user_func( array( &$beanInstance, $propertyMethod ), $propDef->getValue() );
					...
					elseif ( $propType == 'ref' || $propType == 'bean' ) {
						...
						// print_r($dependentBeanInstance);
						
						// echo  "================<br>Deepak22 <br>";


						// print_r(array( $beanInstance, $propertyMethod ));


						/*if(!in_array($propertyMethod ,array('setDataService','setEntityPrototype' ,'setAuthService' ,'setWebServiceUrl' , 'setVenueDAO' , 'setLeaderDAO'))){
